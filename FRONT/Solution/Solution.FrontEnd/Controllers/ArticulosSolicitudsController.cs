...
namespace Solution.FrontEnd.Controllers
{
    public class ArticulosSolicitudsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            List<data.ArticulosSolicitudes> aux = new List<data.ArticulosSolicitudes>();
            using (var cl = new HttpClient())
            {
                ...
                if (res.IsSuccessStatusCode)
                {
                    ...
                    aux = JsonConvert.DeserializeObject<List<data.ArticulosSolicitudes>>(auxres);
                }
            }
            return View(aux);
        }
        ...
        public async Task<IActionResult> Create([Bind("IdSolicitud,IdArticulo")] ArticulosSolicitudes articulosSolicitud)
        {
            if (User == null || !User.Identity.IsAuthenticated)
            {
                return Redirect("Home/Index");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("IdArticuloSolicitud,IdSolicitud,IdArticulo")] ArticulosSolicitudes articulosSolicitud)
        {
            if (User == null || !User.Identity.IsAuthenticated)
            {
                return Redirect("Home/Index");
        ...
        private data.ArticulosSolicitudes GetById(int? id)
        {
            data.ArticulosSolicitudes aux = new data.ArticulosSolicitudes();
            using (var cl = new HttpClient())
            {
                cl.BaseAddress = new Uri(baseurl);
                cl.DefaultRequestHeaders.Clear();
                ...
                {
                    var auxres = res.Content.ReadAsStringAsync().Result;
                    aux = JsonConvert.DeserializeObject<data.ArticulosSolicitudes>(auxres);
                }
            }
            return aux;
        }
